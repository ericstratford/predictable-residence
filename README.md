...
In conclusion, our model is still not where we would like it to be as it is suffering from underfitting a little bit. However, it still performs reasonably well despite this. If we had more time with this project we would definitely try other models to achieve a higher testing accuracy. If we could start our project over from the beginning with the knowledge we have now, we would probably spend much more time exploring and pre-processing the data before we start training any models. One issue we discovered is that while we had a lot of data, it wasn't enough to properly classify the various output classes in our data. So, we had to combine output classes in order to improve our results. The next models we would use are SVMs and neural networks. SVMs work well as classifiers with non-linear decision boundaries, and we can pair them with PCA to reduce the dimensionality of the dataset. Neural network using a multilayer perceptron model are another option since our dataset is large and complex. All things considered, we had fun working on this project and putting the skills we learned in the class to the test.
